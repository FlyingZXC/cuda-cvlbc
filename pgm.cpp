...
- use ::pgm_extract_head to extract the image meta data
(size and bit depth);
- allocate a buffer to store the image data;
- use ::pgm_extract_data to extract the image data to the allocated
buffer.

Writing an image in PGM format to an ouptut file stream
can be done by using ::pgm_insert.

To quickly read/write a PGM image from/to a given file, use
::pgm_read_new() and ::pgm_write(). To to the same from a
buffer in floating point format use ::pgm_read_new_f() and
::pgm_write_f().
...
** ::PgmImage accordingly.
**
** @return error code. The error may be either
** #PGM_INV_HEAD or #PGM_INV_META depending whether the error
...
** according. The buffer @a data should be ::pgm_get_npixels() by
** ::pgm_get_bpp() bytes large.
**
** @return error code.
**/
...
{
	int bpp = pgm_get_bpp(im);
	int data_size = pgm_get_npixels(im);
	int c;
	int good = 1;
	...
	*/
	if (im->is_raw) 
	{
		...
		{
			int i;
			char unsigned *pt = (char unsigned*)data;
			for (i = 0; i < 2 * data_size; i += 2) 
			{
				char unsigned tmp = pt[i];
				pt[i] = pt[i + 1];
				pt[i + 1] = tmp;
			}
		}
	...
	*/
	else 
	{
		int i;
		int unsigned v;
		for (good = 1, i = 0; i < data_size && good; ++i) 
		{
			c = fscanf(f, " %ud", &v);
			if (bpp == 1) 
			{
				*((char unsigned*)data + i) = (char unsigned)v;
			}
			else 
			{
				*((short unsigned*)data + i) = (short unsigned)v;
			}
			good &= c == 1;
		}
	}
...
int pgm_insert(FILE* f, PgmImage const *im, void const *data)
{
	...
	/* write preamble */
	fprintf(f, "P5\n%d\n%d\n%d\n", im->width, im->height, im->max_value);
	...
	{
		int i;
		char unsigned* temp = malloc(2 * data_size);
		memcpy(temp, data, 2 * data_size);
		for (i = 0; i < 2 * data_size; i += 2) 
		{
			char unsigned tmp = temp[i];
			temp[i] = temp[i + 1];
			temp[i + 1] = tmp;
		}
...
** @a data should be freed by means of ::free().
**
...
int pgm_read_new(char const *name, PgmImage *im, char unsigned** data)
{
	...
	if (!f) {
		fprintf(stderr, "Error opening PGM file `%s' for reading.\n", name);
		return -1;
	}

	err = pgm_extract_head(f, im);
	if (err) {
		fclose(f);
		return err;
	}
	...
	if (pgm_get_bpp(im) > 1) {
		fprintf(stderr, "%s\n", "pgm_read_new(): PGM with BPP > 1 not supported");
		return -1;
	}

	*data = (char unsigned*)malloc(pgm_get_npixels(im) * sizeof(char unsigned));
	err = pgm_extract_data(f, im, *data);
...
int pgm_read_new_f(char const *name, PgmImage *im, float** data)
{
	...
	err = pgm_read_new(name, im, &idata);
	if (err) {
		return err;
	}

	npixels = pgm_get_npixels(im);
...
int pgm_write_f(char const *name, float const* data, int width, int height)
{
	int err = 0;
	int k;
	float min = +INFINITY_F;
	float max = -INFINITY_F;
	float scale;

	char unsigned *buffer = (char unsigned *)malloc(sizeof(float) * width * height);
	...
	scale = 255 / (max - min + EPSILON_F);

	for (k = 0; k < width * height; ++k) {
		buffer[k] = (char unsigned)((data[k] - min) * scale);
	}

	err = pgm_write(name, buffer, width, height);
